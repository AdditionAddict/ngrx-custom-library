...
import { of, merge, ReplaySubject, throwError, timer, Observable } from 'rxjs';
import { delay, first, mergeMap, tap, catchError } from 'rxjs/operators';

import { NxaEntityActionFactory } from '../../lib/actions/entity-action-factory';
import { Logger } from '../../lib/utils/interfaces';
import { NxaEntityOp, makeNxaErrorOp, makeNxaOfflineErrorOp } from '../../lib/actions/entity-op';
...
import { NxaOfflineServiceError, NxaEntityActionOfflineServiceError } from '../../lib/offline-services/offline-service-error'
...
describe('NxaEntityEffects (normal testing)', () => {
    ...
    let offlineService: TestDataService;
    let onlineCheckService: TestOnlineCheckService;
    ...
    beforeEach(() => {
        ...
        TestBed.configureTestingModule({
            providers: [
                ...
                { provide: NxaOnlineCheckService, useClass: TestOnlineCheckService },
        ...
        offlineService = TestBed.get(NxaEntityOfflineService);
        onlineCheckService = TestBed.get(NxaOnlineCheckService);
    });
    ...
    describe('Online mode', () => {
        ...
        it('should return a QUERY_ALL_ERROR when offline service fails', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create('Hero', NxaEntityOp.QUERY_ALL);
    ...
    describe('Offline mode', () => {
        beforeEach(() => {
            onlineCheckService.setOnlineStatus(false);
        });

        it('should return a QUERY_ALL_OFFLINE_SUCCESS with the heroes on success', (done: DoneFn) => {
            const hero1 = { id: 1, name: 'A' } as Hero;
            const hero2 = { id: 2, name: 'B' } as Hero;
            const heroes = [hero1, hero2];
            offlineService.setResponse('getAll', heroes);

            const action = nxaEntityActionFactory.create('Hero', NxaEntityOp.QUERY_ALL);
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.QUERY_ALL_OFFLINE_SUCCESS,
                heroes
            );
        ...
        it('should perform QUERY_ALL when dispatch custom tagged action', (done: DoneFn) => {
            ...
            const completion = nxaEntityActionFactory.createFromAction(action, {
                entityOp: NxaEntityOp.QUERY_ALL_OFFLINE_SUCCESS,
                data: heroes,
            });
        ...
        it('should return a QUERY_ALL_OFFLINE_ERROR when data service fails', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create('Hero', NxaEntityOp.QUERY_ALL);
            const error = makeNxaOfflineServiceError(`Test message`);
            const completion = makeEntityOfflineErrorCompletion(action, error);
            actions$.next(action);
            offlineService.setErrorResponse('getAll', error);

            expectCompletion(completion, done);
            expect(completion.payload.entityOp).toEqual(NxaEntityOp.QUERY_ALL_OFFLINE_ERROR);
        });

        it('should return a QUERY_BY_KEY_OFFLINE_SUCCESS with a hero on success', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
            const action = nxaEntityActionFactory.create('Hero', NxaEntityOp.QUERY_BY_KEY, 1);
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.QUERY_BY_KEY_OFFLINE_SUCCESS,
                hero
            );

            actions$.next(action);
            offlineService.setResponse('getById', hero);

            expectCompletion(completion, done);
        });

        it('should return a QUERY_BY_KEY_OFFLINE_ERROR when data service fails', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.QUERY_BY_KEY,
                42
            ...
            actions$.next(action);
            offlineService.setErrorResponse('getById', error);

            expectCompletion(completion, done);
        });

        it('should return a QUERY_MANY_OFFLINE_SUCCESS with selected heroes on success', (done: DoneFn) => {
            const hero1 = { id: 1, name: 'BA' } as Hero;
            const hero2 = { id: 2, name: 'BB' } as Hero;
            const heroes = [hero1, hero2];
            ...
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.QUERY_MANY_OFFLINE_SUCCESS,
                heroes
            );

            actions$.next(action);
            offlineService.setResponse('getWithQuery', heroes);

            expectCompletion(completion, done);
        });

        it('should return a QUERY_MANY_OFFLINE_ERROR when online service fails', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create('Hero', NxaEntityOp.QUERY_MANY, {
                name: 'B',
            });
            const error = makeNxaOfflineServiceError(`Test message`);
            ...
            actions$.next(action);
            offlineService.setErrorResponse('getWithQuery', error);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_ADD_ONE_OFFLINE_SUCCESS (Optimistic) with the hero on success', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
            ...
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_ADD_ONE_OFFLINE_SUCCESS,
                hero,
                { isOptimistic: true }
            );

            actions$.next(action);
            offlineService.setResponse('add', hero);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_ADD_ONE_OFFLINE_SUCCESS (Pessimistic) with the hero on success', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
        ...
        it('should return a SAVE_ADD_ONE_OFFLINE_ERROR when data service fails', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_ADD_ONE,
            ...
            actions$.next(action);
            offlineService.setErrorResponse('add', error);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_DELETE_ONE_OFFLINE_SUCCESS (Optimistic) on success with delete id', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_DELETE_ONE,
                42,
            ...
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_DELETE_ONE_OFFLINE_SUCCESS,
                42,
                { isOptimistic: true }
            );

            actions$.next(action);
            offlineService.setResponse('delete', 42);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_DELETE_ONE_OFFLINE_SUCCESS (Pessimistic) on success', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_DELETE_ONE,
                42
        ...
        it('should return a SAVE_DELETE_ONE_OFFLINE_ERROR when data service fails', (done: DoneFn) => {
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_DELETE_ONE,
                42
            ...
            actions$.next(action);
            offlineService.setErrorResponse('delete', error);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_UPDATE_ONE_OFFLINE_SUCCESS (Optimistic) with the hero on success', (done: DoneFn) => {
            const updateEntity = { id: 1, name: 'A' };
            const update = { id: 1, changes: updateEntity } as Update<Hero>;
            const updateResponse = { ...update, changed: true };
            ...
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_UPDATE_ONE_OFFLINE_SUCCESS,
                updateResponse,
                { isOptimistic: true }
            );

            actions$.next(action);
            offlineService.setResponse('update', updateEntity);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_UPDATE_ONE_OFFLINE_SUCCESS (Pessimistic) with the hero on success', (done: DoneFn) => {
            const updateEntity = { id: 1, name: 'A' };
            const update = { id: 1, changes: updateEntity } as Update<Hero>;
            const updateResponse = { ...update, changed: true };
        ...
        it('should return a SAVE_UPDATE_ONE_OFFLINE_ERROR when data service fails', (done: DoneFn) => {
            const update = { id: 1, changes: { id: 1, name: 'A' } } as Update<Hero>;
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_UPDATE_ONE,
            ...
            actions$.next(action);
            offlineService.setErrorResponse('update', error);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_UPSERT_ONE_OFFLINE_SUCCESS (Optimistic) with the hero on success', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
            ...
            const completion = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_UPSERT_ONE_OFFLINE_SUCCESS,
                hero,
                { isOptimistic: true }
            );

            actions$.next(action);
            offlineService.setResponse('upsert', hero);

            expectCompletion(completion, done);
        });

        it('should return a SAVE_UPSERT_ONE_OFFLINE_SUCCESS (Pessimistic) with the hero on success', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
        ...
        it('should return a SAVE_UPSERT_ONE_OFFLINE_ERROR when data service fails', (done: DoneFn) => {
            const hero = { id: 1, name: 'A' } as Hero;
            const action = nxaEntityActionFactory.create(
                'Hero',
                NxaEntityOp.SAVE_UPSERT_ONE,
            ...
            actions$.next(action);
            offlineService.setErrorResponse('upsert', error);
...
/** Make an NxaEntityOfflineService error */
function makeEntityOfflineErrorCompletion(
    /** The action that initiated the data service call */
    originalAction: NxaEntityAction,
    /** error produced by the NxaEntityDataService */
    error: NxaOfflineServiceError
) {
    const errOp = makeNxaOfflineErrorOp(originalAction.payload.entityOp);

    // Entity Error Action
    const eaFactory = new NxaEntityActionFactory();
    return eaFactory.create<NxaEntityActionOfflineServiceError>('Hero', errOp, {
        originalAction,
        error,
    });
}
/** make error produced by the NxaOfflineServiceError */
function makeNxaOfflineServiceError(
    error: string
) {
    return new NxaOfflineServiceError(error);
}
...
export class TestOnlineCheckService {
    isOnline = true;
    ...
    online$() {
        return new Observable(sub => {
            if (this.shouldThrowError) {
                ...
                sub.next(this.isOnline)
                sub.complete()
            }
        }).pipe(
            catchError(error => timer(1).pipe(mergeMap(() => throwError(error))))
        )
    }
