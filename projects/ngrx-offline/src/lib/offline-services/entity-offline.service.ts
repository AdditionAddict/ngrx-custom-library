...
import { NxaDefaultOfflineServiceFactory } from './default-offline.service';
 ...
 */
@Injectable()
export class NxaEntityOfflineService {
    ...
    constructor(protected defaultOfflineServiceFactory: NxaDefaultOfflineServiceFactory) { }
    ...
    getService<T>(entityName: string): NxaEntityCollectionDataService<T> {
        ...
        if (!service) {
            service = this.defaultOfflineServiceFactory.create(entityName);
